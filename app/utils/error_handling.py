...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    json_invalid = next((err for err in errors if err.get("type") == "json_invalid"), None)
    if json_invalid:
        return JSONResponse(
            status_code=422,
            content={
                "success": False,
                "result": None,
                "error_message": json_invalid.get("ctx", {}).get("error", "Invalid JSON"),
    ...
    is_missing_file = any(
        "files" in err["loc"] and err["type"] == "missing"
        for err in errors
    )
    if is_missing_file:
        return JSONResponse(
